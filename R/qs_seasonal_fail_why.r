...
#' @param robust_sa Logical scalar indicating if original series adjusted for extremes is included 
#'        in testing. Default is \code{FALSE}.
#' @param return_both Logical scalar indicating whether the calling function will return both the 
#         test results and why the test failed or produced a warning. Default is \code{FALSE}.
#' @return A text string denoting if the series passed or failed the QS diagnostic. 
#'
...
#' @export
qs_seasonal_fail_why <- function(udg_list = NULL, test_full = TRUE, test_span = TRUE, 
    p_limit_warn = 0.05, robust_sa = TRUE, return_both = FALSE) {
    ...
    if (test_span) {
        ...
        # \code{n_test} if necessary
        if (index_qs > 0) {
            if (udg_list[["qssori"]][2] > p_limit_warn) {
                if (nchar(this_out_string) > 0) {
        ...
        if (robust_sa) {
            ...
            # \code{n_test} if necessary
            if (index_qs > 0) {
                if (udg_list[["qssorievadj"]][2] > p_limit_warn) {
                  if (nchar(this_out_string) > 0) {
