...
#' @return a numeric vector with the degrees of freedom, F statistic, and probability  
...
#' @export
get_seasonal_ftest_all <- function(seas_obj = NULL, this_series = "b1", use_seasonal = TRUE) {
    # Author: Brian C. Monsell (OEUS) Version 4.3, 6/21/2024
    ...
    if (this_index > 0) {
        seasonal_ftest <- as.vector(seasonal::udg(seas_obj, this_key))
    } else {
        seasonal_diff <- as.numeric(sautilities::get_udg_entry(seas_obj,"seasonaldiff"))
        this_model <- this_udg$arimamdl
        if (seasonal_diff > 0) {
            this_new_model          <- paste0(substr(this_model,1,9), " 0 0)")
        } else {
            if (substring(this_model, 8, 15) == "(1 0 1)") {
                this_new_model      <- paste0(substr(this_model,1,7), "(1 0 0)")
            } else {
        ...
        seas_obj_static <- seasonal::static(seas_obj, evaluate = TRUE, test = FALSE)
        ...
        seasonal_ftest <- 
            as.vector(seasonal::udg(seas_obj_static_update, "ftest$Seasonal"))
    }
    
    return(seasonal_ftest)
}