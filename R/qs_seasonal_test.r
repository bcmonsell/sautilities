...
#' @param return_this Character string; what the function returns - \code{'test'} returns  
#'        test results, \code{'why'} returns why the test failed or received a warning, or 
#'        \code{'both'}. Default is \code{'test'}.
#' @return A text string denoting if the series passed or failed tests for seasonality using 
#'         the QS diagnostics. Can test the entire series or the last 8 years or both. 
#'
...
#' @export
qs_seasonal_test <- function(seas_obj = NULL, test_full = TRUE, test_span = TRUE,  
    p_limit_pass = 0.01, p_limit_warn = 0.05, robust_sa = TRUE, return_this = "test") {
    ...
    # Check if full span of series has QS statistics for original series.  If none found, return
    # \code{'no_qs_stats'}
    if (test_full) {
        args_full <- c("qsori", "qsorievadj")
        n_test <- 0
        for (i in 1:2) {
            index_qs <- get_udg_index(udg_list, args_full[i])
            if (index_qs > 0) {
                n_test <- n_test + 1
            }
        }
    ...
    # Check if reduced (8-year) span of series has QS statistics for original series.  
    # If none found, return \code{'no_span_qsrsd_stats'}
    if (test_span) {
        args_span <- c("qssori", "qssorievadj")
        n_test <- 0
        for (i in 1:2) {
            index_qs <- get_udg_index(udg_list, args_span[i])
            if (index_qs > 0) {
                n_test <- n_test + 1
            }
        }
    ...
    if (test_full) {
        ...
        if (index_qs > 0) {
            if (udg_list[["qsori"]][2] <= p_limit_warn) {
                if (udg_list[["qsori"]][2] >= p_limit_pass) {
                  n_warn <- n_warn + 1
                }
            } else {
                n_fail <- n_fail + 1
        ...
        # Test original series adjusted for extremes
        if (robust_sa) {
            # get index of the QS statistic for the original series adjusted for extremes
            index_qs <- get_udg_index(udg_list, "qsorievadj")
            
            # If this QS exists, update \code{n_warn} and \{n_fail} if necessary
            if (index_qs > 0) {
                if (udg_list[["qsorievadj"]][2] <= p_limit_warn) {
                  if (udg_list[["qsorievadj"]][2] >= p_limit_pass) {
                    n_warn <- n_warn + 1
                  }
                } else {
                  n_fail <- n_fail + 1
    ...
    if (test_span) {
        ...
        # Test original series adjusted for extremes
        if (robust_sa) {
            # get index of the QS statistic for the span of the original series with extremes 
            # removed
            index_qs <- get_udg_index(udg_list, "qssorievadj")
    ...
    if (is_fail | is_fail_span) {
        this_test <- "fail"
        this_why <- qs_seasonal_fail_why(udg_list, test_full = is_fail, test_span = is_fail_span, 
    ...
    if (is_warn | is_warn_span) {
        ...
        this_why <- 
            paste0(this_why, qs_seasonal_warn_why(udg_list, test_full = is_warn, 
                   test_span = is_warn_span, p_limit_pass = p_limit_pass, 
                   robust_sa = robust_sa, return_both = is_both))
    }
